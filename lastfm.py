...
def current_track_url() -> str:
    return (
        ...
        f"?method=user.getrecenttracks&nowplaying=true&user={userName}"
        f"&api_key={get_lastfm_api_key()}"
    )
runCheck = True
waitTime = 15
...
async def lastfm_api(client, message):
    ...
    await message.edit(
        "✅ <b>Last.fm API key saved.</b> Restart Zelretch to start automatic track monitoring."
    )
...
async def nowplayed(client, message):
    ...
    if not get_lastfm_api_key():
        return await message.edit(
            "⚠️ <b>Last.fm API key is not configured.</b>\n"
