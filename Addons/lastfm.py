"""Zelretch Addon: Last.fm Status

Displays and optionally auto-updates your currently playing Last.fm track.
...
"""
ZELRETCH_MODULE_INFO = {'title': 'Last.fm Status', 'icon': '🎧', 'category': 'Media', 'description': 'Displays and optionally auto-updates your currently playing Last.fm track.', 'developer': 'Siam Chowdhury', 'github': 'https://github.com/ChowdhurySiam', 'telegram': 'https://t.me/Ch0wdhury_Siam'}
...
LASTFM_KEY_PATH = 'userdata/lastfm_api_key'
...
def get_lastfm_api_key() -> str:
    ...
    if key:
        return key
    try:
        with open(LASTFM_KEY_PATH, 'r', encoding='utf-8') as file:
            return file.read().strip()
    except FileNotFoundError:
        return ''

def current_track_url() -> str:
    return f'https://ws.audioscrobbler.com/2.0/?method=user.getrecenttracks&nowplaying=true&user={userName}&api_key={get_lastfm_api_key()}'
runCheck = True
waitTime = 15
...
def checkForNewSong():
    global runCheck, waitTime
    if userName != 'None' and get_lastfm_api_key():
        while runCheck:
            try:
                currentTrackXML = None
                while currentTrackXML is None:
                    currentTrackXML = send_request(current_track_url())
...
try:
    try:
        ...
        currentUsername = 'None'
    if currentUsername == 'None' or not get_lastfm_api_key():
        pass
    else:
...
@Client.on_message(zel_command('lastfm_api', 'LastFM', os.path.basename(__file__), '[api_key]') & zel_sudo())
async def lastfm_api(client, message):
    message = await who_message(client, message)
    parts = message.text.split(maxsplit=1)
    if len(parts) < 2 or not parts[1].strip():
        status = 'configured' if get_lastfm_api_key() else 'not configured'
        return await message.edit(f'🎧 <b>Last.fm API key is {status}.</b>')
    os.makedirs('userdata', exist_ok=True)
    with open(LASTFM_KEY_PATH, 'w', encoding='utf-8') as file:
        file.write(parts[1].strip())
    await message.edit('✅ <b>Last.fm API key saved.</b> Restart Zelretch to start automatic track monitoring.')

@Client.on_message(zel_command('nowplayed', 'LastFM', os.path.basename(__file__)) & zel_sudo())
async def nowplayed(client, message):
    message = await who_message(client, message)
    if not get_lastfm_api_key():
        return await message.edit('⚠️ <b>Last.fm API key is not configured.</b>\nUse <code>.lastfm_api YOUR_KEY</code> or set <code>LASTFM_API_KEY</code>.')
    try:
        ...
        try:
            ...
            await message.edit(f'[❌] Error while creating the file: {e}')

@Client.on_message(zel_command('lastfm_config', 'LastFM', os.path.basename(__file__), '[LastFM Nickname] [Username/ID Channel] [ID Message] [Autostart: True/False]') & zel_sudo())
...
@Client.on_message(zel_command('autoplayed', 'LastFM', os.path.basename(__file__)) & zel_sudo() | filters.command('last_fm_trigger_start', prefixes='') & zel_sudo() & filters.chat('me'))
async def autoplayed(client, message):
    ...
    while True:
        ...
        if str(cache) == str(text):
            ...
            try:
                await client.edit_message_text(chat_id=channel, message_id=id_in_channel_telegram, text=f'[🎶] {text}')
