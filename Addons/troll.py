...
async def hack(client, message):
    ...
    perc = 0
    while perc < 100:
        text = '👮 Simulated security scan in progress...' + str(perc) + '%'
    ...
    perc = 0
    while perc < 100:
        text = '⬇️ Downloading imaginary data...' + str(perc) + '%'
        await message.edit(str(text))
        perc += random.randint(1, 5)
        await asyncio.sleep(0.15)
    await asyncio.sleep(1)
    text = '🗂 Found a folder full of questionable decisions!'
...
async def drugs(client, message):
    ...
    perc = 0
    while perc < 100:
        text = '🔎 Searching the fictional inventory ' + str(perc) + '%'
        await message.edit(str(text))
        perc += random.randint(1, 3)
        await asyncio.sleep(0.1)
    text = '📦 Found a suspiciously empty box'
    await message.edit(str(text))
    await asyncio.sleep(3)
    text = '🧪 Running the harmless simulation'
    await message.edit(str(text))
    await asyncio.sleep(5)
    drugsss = ['😳 Simulation complete. Make safer choices.', '🥴 The simulation rejected that attempt. Try another harmless command.', '😖 Simulation failed because the fictional input exceeded its limit.', '😌 Simulation completed successfully.']
...
async def police(client, message):
    ...
    while need_duration != duration:
        ...
        await asyncio.sleep(0.3)
        duration += 1
    await message.edit('**Stay where you are!**\nThe **🚨 police animation 🚨** has arrived.')
end_message = '**1000 - 7 complete. Simulation finished.**'
...
async def stupid(client, message):
    ...
    animation_chars = ['<b>YOUR BRAIN</b> ➡️ 🧠\n\n🧠         (^_^)🗑', '<b>YOUR BRAIN</b> ➡️ 🧠\n\n🧠       (^_^)  🗑', '<b>YOUR BRAIN</b> ➡️ 🧠\n\n🧠     (^_^)    🗑', '<b>YOUR BRAIN</b> ➡️ 🧠\n\n🧠   (^_^)      🗑', '<b>YOUR BRAIN</b> ➡️ 🧠\n\n🧠 (^_^)        🗑', '<b>YOUR BRAIN</b> ➡️ 🧠\n\n🧠< (^_^ <)         🗑', '<b>YOUR BRAIN</b> ➡️ 🧠\n\n(> ^_^)>🧠         🗑', '<b>YOUR BRAIN</b> ➡️ 🧠\n\n  (> ^_^)>🧠       🗑', '<b>YOUR BRAIN</b> ➡️ 🧠\n\n    (> ^_^)>🧠     🗑', '<b>YOUR BRAIN</b> ➡️ 🧠\n\n      (> ^_^)>🧠   🗑', '<b>YOUR BRAIN</b> ➡️ 🧠\n\n        (> ^_^)>🧠 🗑', '<b>YOUR BRAIN</b> ➡️ 🧠\n\n          (> ^_^)>🧠🗑', '<b>YOUR BRAIN</b> ➡️ 🧠\n\n           (> ^_^)>🗑', '<b>YOUR BRAIN</b> ➡️ 🧠\n\n           < (^_^ <)🗑']
...
async def cell(client, message):
    ...
    animation_chars = ['<b>Connecting To Telegram Headquarters...</b>', '<b>Call Connected.</b>', '<b>Telegram: Hello This is Telegram HQ. Who is this?</b>', f'<b>Me: Yo this is FoxU, Please Connect me to my lil bro, Pavel Durov </b>', '<b>User Authorised.</b>', '<b>Calling Shivamani </b>  <code>At +916969696969</code>', '<b>Private  Call Connected...</b>', '<b>Me: Hello Sir, Please Ban This Telegram Account.</b>', '<b>Shivamani : May I Know Who Is This?</b>', f'<b>Me: Yo Brah, I Am FoxU</b>', "<b>Shivamani : OMG!!! Long time no see, Wassup cat...\nI'll Make Sure That Guy Account Will Get Blocked Within 24Hrs.</b>", '<b>Me: Thanks, See You Later Brah.</b>', "<b>Shivamani : Please Don't Thank Brah, Telegram Is Our's. Just Gimme A Call When You Become Free.</b>", '<b>Me: Is There Any Issue/Emergency???</b>', '<b>Shivamani : Yes Sur, There Is A Bug In Telegram v69.6.9.\nI Am Not Able To Fix It. If Possible, Please Help Fix The Bug.</b>', '<b>Me: Send Me The App On My Telegram Account, I Will Fix The Bug & Send You.</b>', '<b>Shivamani : Sure Sur \nTC Bye Bye :)</b>', '<b>Private Call Disconnected.</b>']
    for i in animation_ttl:
        await asyncio.sleep(animation_interval)
...
async def kill(client, message):
    ...
    animation_chars = ['FIRE', '(>_<)====>', '====>____________', '======>__________', '========>', '==========>', '============>', '==============>', '==================>', '======>;(^_^)>', '(-_-) DED', '<b>Target eliminated in the simulation.</b>']
...
async def ZV(client, message):
    message = await who_message(client, message)
    rofl_list = {'Z': 'Z', 'z': 'Z', 'S': 'Z', 's': 'Z', 'V': 'V', 'v': 'V', 'O': 'O', 'o': 'O'}
