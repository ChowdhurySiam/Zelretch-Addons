"""Zelretch Addon: Account Insights

Displays detailed public information and activity estimates for a Telegram account.
...
def calculate_age(date_str):
    try:
        ...
        return f'Age calculation error: {date_str} -> {str(e)}'

@Client.on_message(zel_command('aboutacc', 'AccountData', os.path.basename(__file__), '[username/reply]') & zel_sudo())
async def aboutacc_handler(client, message):
    message = await who_message(client, message)
    reply = message.reply_to_message
    args = message.text.split(maxsplit=1)[1] if len(message.text.split(maxsplit=1)) > 1 else ''
    if not args and (not reply):
        return await message.edit('<emoji id=5248988671855576740>🚫</emoji> <b>Specify a user or reply to a message.</b>')
    await message.edit('<emoji id=5222108309795908493>✨</emoji><i> </i><b>Looking up account information...</b>')
    try:
        if reply:
        ...
        user_id = user.id
        dc_id = user.dc_id if hasattr(user, 'dc_id') and user.dc_id else 'Unknown'
        ...
        await message.edit(f'<emoji id=5276489300207217985>✅</emoji> <b>Account information for {user_name}</b>:\n\n<emoji id=5264892613630111886>💎</emoji> <b>ID:</b> <code>{user_id}</code>\n<emoji id=5258466470676940666>✈️</emoji> <b>Data center:</b> <code>{dc_id}</code>\n<emoji id=5276489300207217985>✅</emoji> <b>Estimated registration date:</b> <code>{reg_date}</code>\n<emoji id=5235588635885054955>🎲</emoji> <b>Estimated account age:</b> <code>{age_str}</code>')
    except Exception as e:
        await message.edit(f'<emoji id=5433992383154824484>🚫</emoji> <b>An error occurred:</b> <code>{str(e)}</code>')
