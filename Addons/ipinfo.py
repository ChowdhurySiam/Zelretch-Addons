...
"""
ZELRETCH_MODULE_INFO = {'title': 'IP Information', 'icon': '🌐', 'category': 'Information', 'description': 'Displays location, network, timezone, and provider data for an IP address.', 'developer': 'Siam Chowdhury', 'github': 'https://github.com/ChowdhurySiam', 'telegram': 'https://t.me/Ch0wdhury_Siam', 'undo': '.undo (reply to the Addon output)'}
import os
import re
...
async def get_location_by_ip(ip_address):
    """Retrieve geographic information for an IP address."""
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if data.get('status') == 'fail':
                ...
                return get_text(Module_Name, 'data', LANGUAGES=LANGUAGES, ip=data.get('query', ''), country=data.get('country', ''), timezone=data.get('timezone', ''), city=data.get('city', ''), region=data.get('regionName', ''), coordinates=f"{data.get('lat', '')}, {data.get('lon', '')}", provider=data.get('isp', ''))
        else:
            return get_text(Module_Name, 'no_data', LANGUAGES=LANGUAGES)
    except Exception:
        return get_text(Module_Name, 'no_data', LANGUAGES=LANGUAGES)
