...
async def sysinfo(client, message):
    ...
    try:
        ...
        sys_info = f'<b>🖥 System information:</b>\n<b>📍 Host:</b> <code>{hostname}</code>\n<b>🌐 IP address:</b> <code>{ip_address}</code>\n<b>🧠 Memory:</b>\n - Total: <code>{total_memory:.2f} GB</code>\n - Used: <code>{used_memory:.2f} GB</code>\n - Free: <code>{free_memory:.2f} GB</code>\n<b>🔢 CPU cores:</b> <code>{cpu_count}</code>\n<b>💽 Disk:</b>\n - Total: <code>{total_disk:.2f} GB</code>\n - Used: <code>{used_disk:.2f} GB</code>\n - Free: <code>{free_disk:.2f} GB</code>'
        await message.edit(sys_info)
    except Exception as e:
        await message.edit(f'<code>Error while retrieving system information: {str(e)}</code>')
