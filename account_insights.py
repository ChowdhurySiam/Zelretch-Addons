...
async def aboutacc_handler(client, message):
    ...
    if not args and not reply:
        return await message.edit("<emoji id=5248988671855576740>🚫</emoji> <b>Укажите пользователя или ответьте на сообщение!</b>")

    await message.edit("<emoji id=5222108309795908493>✨</emoji><i> </i><b>Ищу информацию...</b>")
    ...
    try:
        ...
        user_id = user.id
        dc_id = user.dc_id if hasattr(user, "dc_id") and user.dc_id else "Неизвестно"
        ...
        await message.edit(
            f"<emoji id=5276489300207217985>✅</emoji> <b>Информация о {user_name}</b>:\n\n"
            f"<emoji id=5264892613630111886>💎</emoji> <b>ID:</b> <code>{user_id}</code>\n"
            f"<emoji id=5258466470676940666>✈️</emoji> <b>Дата-центр:</b> <code>{dc_id}</code>\n"
            f"<emoji id=5276489300207217985>✅</emoji> <b>Дата регистрации:</b> <code>{reg_date}</code>\n"
            f"<emoji id=5235588635885054955>🎲</emoji> <b>Возраст аккаунта:</b> <code>{age_str}</code>"
        )

    except Exception as e:
        await message.edit(f"<emoji id=5433992383154824484>🚫</emoji> <b>Произошла ошибка:</b> <code>{str(e)}</code>")
